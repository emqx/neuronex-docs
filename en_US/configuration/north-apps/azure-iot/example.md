...
Here, we registered a device *client-005* for EMQX Neuron.
...
We will use [Azure IoT Explorer] to monitor messages between Azure IoT Hub and EMQX Neuron.
...
Click **Monitoring**, then select the *modbus-tcp* node and the *group* group. We see that EMQX Neuron reports a initial value *0* for *tag0*.
...
In Azure IoT Explorer, click **Cloud-to-device message** to send a write request to EMQX Neuron, which writes value *42* to the tag *tag0*.
<figure align="center">
  <img src="./assets/azure_neuron_write.png" style="border:thin solid #E0DCD9; width: 60%" alt="Azure IoT Explorer write">
</figure>

Now we can see that EMQX Neuron updates *tag0* correctly in the **Data monitoring** tab.
<figure align="center">

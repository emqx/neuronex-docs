...
Navigate to **Data Collection -> North Apps** and click **Add Application** to add a Kafka node.
...
If there are any issues with device operation, you can click the DEBUG log chart. The system will automatically print DEBUG level logs for that node and switch back to the system default log level after ten minutes. Later, you can click **Administration** -> **Logs** to view logs and perform troubleshooting. For a detailed interpretation of system logs, see [Managing Logs](../../../admin/log-management.md).

## Integrating with Microsoft Fabric

The following applies when delivering industrial data collected by EMQX Neuron to **Eventstream** in **Microsoft Fabric** using a Kafka-compatible endpoint. For end-to-end topology and Eventstream canvas steps, refer to [Microsoft Fabric documentation](https://learn.microsoft.com/fabric/).
...
- **Close the OT → IT gap**: EMQX Neuron connects to Modbus, OPC UA, PLCs, and other industrial protocols at the edge, shapes time-series data into structured JSON, and sends it over Kafka into Fabric—without ad-hoc collectors per business line.
...
### Two ways for EMQX Neuron to reach Fabric

EMQX Neuron can send data to Fabric Eventstream (Kafka-compatible endpoint) via the paths below. **Both can coexist**; choose based on whether you need SQL stream processing at the edge.
...
| EMQX Neuron northbound Kafka field | Description | Example (must match your **Microsoft Fabric** / **Azure** portal) |
| ------------------------------- | ------------ | ----------------------------------------------------------------- |
...
After configuration, confirm delivery using EMQX Neuron runtime statistics or DEBUG logs, and Fabric Eventstream **Data preview** (or downstream Eventhouse queries) to verify that messages keep arriving.

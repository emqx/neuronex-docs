...
Add global static tags through custom data format. These static tags will be carried in all driver group data reporting. For example, if EMQX Neuron is deployed on a gateway hardware, you can add the gateway's SN number, IP address, and location information to all driver groups.
...
If the static tags added are shared by all collection groups in the EMQX Neuron instance, you can select **All Collection Groups** to add static tags, or add static tags in the **Custom** custom format, as shown in [Custom Format: Example Two](#example-two).
...
::: tip
Before EMQX Neuron version 2.4.5, the write request topic was hard-coded to **/neuron/{random_str}/write/req**.
:::
...
Since EMQX Neuron version 2.6.0, write requests also support writing multiple tags at
...
::: tip
Before EMQX Neuron version 2.4.5, the write response topic was hard-coded to **/neuron/{random_str}//write/resp**.
:::

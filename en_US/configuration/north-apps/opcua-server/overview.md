...
EMQX Neuron supports using OPC UA Server as a northbound application, allowing southbound device data to be exposed to upper-level systems or third-party clients via OPC UA services. Through the OPC UA Server, external systems can subscribe to data changes, read real-time points, and send control commands.
...
- Tags are mapped to Variable nodes, with DataType mapped from EMQX Neuron's type to OPC UA types (Double, Int32, Boolean, String, etc.).

All southbound nodes are under the EMQX Neuron node. NodeId follows the format `ns=1;s=[device].[group].[tag]`, e.g., `ns=1;s=modbus-tcp-1.group-1.temperature`, where ns=1 is the EMQX Neuron namespace.

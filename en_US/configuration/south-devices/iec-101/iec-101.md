...
On **Data Collection → South Devices**, click **Add Device** and select **IEC60870-5-101** as the driver type.
...
After adding and configuring the driver, you need to establish communication between the device and EMQX Neuron by adding groups and points to the southbound driver.
...
IEC 60870-5-101 supports the point to report the value when the point changes. On the EMQX Neuron side, you need to configure the point with the Subscribe attribute. When the device sends the changed data point to EMQX Neuron, EMQX Neuron will immediately send this change message to the corresponding northbound application.
...
::: tip
For tags that carry a timestamp and a quality flag, the quality and time can optionally be included when the data is published northbound.
:::

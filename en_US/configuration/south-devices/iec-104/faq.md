...
* The value read from the device does not match the data type in EMQX Neuron. For example, if the device returns a single point when IOA=100, but the Float type is configured in EMQX Neuron, such an error will occur.

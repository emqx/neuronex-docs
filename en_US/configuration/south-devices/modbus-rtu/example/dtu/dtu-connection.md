...
| **Remote Server Addr** | When DTU acts as a client, fill in the address bound to EMQX Neuron Modbus.|
| **Local/Remote Port Number** | When DTU acts as a client, fill in the port bound to EMQX Neuron Modbus. When DTU acts as a server, use the port of DTU.|

:::tip
DTU usually supports serial port heartbeats, or enables network heartbeats and registration packets. These features cannot be used in the standard Modbus protocol, and EMQX Neuron currently is not compatible with these features. When using EMQX Neuron to connect to a DTU, be sure to turn off these options.
:::
...
## Establish Client Connection with EMQX Neuron

This section mainly describes the configuration of EMQX Neuron and DTU when EMQX Neuron serves as the Client and DTU serves as Server.

As a Client, EMQX Neuron initiates connection requests to DTU actively. The user needs to ensure the network connectivity of EMQX Neuron -> DTU.

### Configure DTU Server

First, you need to configure the parameters of the connection between the DTU and the serial port, and second, you need to configure the parameters of the Socket for the connection between the DTU and the EMQX Neuron, as shown in the figure below.
...
While configuring the EMQX Neuron's southbound driver, you'll need to input the IP address of the server-side DTU, as illustrated in the figure below.
...
## Establish Server Connection with EMQX Neuron

This section mainly describes the configuration of EMQX Neuron and DTU when EMQX Neuron serves as the Server and DTU serves as the Client.

As a Client, DTU initiates connection requests to EMQX Neuron. Users need to ensure the network connectivity of DTU -> EMQX Neuron. This connection mode can generally be used in the following scenarios. When some DTU uses 4G to access the Internet, EMQX Neuron cannot actively connect to DTU, so EMQX Neuron can only choose Server mode and actively connect to EMQX Neuron by DTU.

Besides, When EMQX Neuron and DTU are not in the same LAN, you can map the LAN IP and port of the EMQX Neuron operating environment to the WAN, and use EMQX Neuron as the server end and DTU as the client end. 
...
* **Remote server address**: Fill in the IP address of EMQX Neuron (running as the server);

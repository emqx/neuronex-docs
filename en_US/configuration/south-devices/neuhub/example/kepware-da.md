# Example of KEPServerEX OPCDA Server Connection
...
When the OPC DA node is in a normal connected state, all OPC DA tags can be exported in the format of the EMQX Neuron EX tag template file. To do this, click the `Export` button in the `Actions` column of the OPC DA node whose tags need to be exported.  

# Connect to S7-1200 PLC

This article will use the Siemens S7 ISOTCP driver to connect to a Siemens S7-1200 PLC and read/write the tag values in the PLC.

![connect-s71200](./assets/c1200.jpg)

The Siemens S7 ISOTCP driver can connect to the PLC via the local network or the Internet, but note that if the PLC and the EMQX Neuron server are not on the same local network, you need to configure port mapping in the PLC.

## S7-1200 PLC Tags

This article assumes that you can connect to the PLC using the TIA Portal software. On how to set up the connection, see [Tailor S7 1200/1500 PLC for Effective ISOTCP Driver Use](../../plc/siemens-s7-1200-1500.md)
...
* In EMQX Neuron, click **Data Collection** -> **South Devices**, click **Add Device** to create a node that connects to the 1200 PLC by selecting the **Siemens S7 ISOTCP** driver.

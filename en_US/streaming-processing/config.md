...
EMQX Neuron has made compatibility changes to support exporting rules from older versions of EMQX Neuron and importing them on EMQX Neuron 3.4. The rules can still run normally. After that, you can add connectors and manually modify the Source and Sink configurations.
...
A schema is a set of metadata that defines the structure of the data. For example, the Protobuf format uses .proto files as the schema to define the transmitted data format. Currently, EMQX Neuron only supports protobuf and custom modes.
...
This section takes the Protobuf format as an example to explain how to set the codec format in EMQX Neuron.
...
1. In the EMQX Neuron interface, click **Data Stream Processing** -> **Configuration** -> **Mode**, and click the **Create Mode** button in the upper right corner.
...
When adding files as data sources or through batch rules, you need to upload the prepared files to EMQX Neuron. The steps are as follows:

Log in to EMQX Neuron, click **Data Processing** -> **Configuration** -> **Files Management**, and click the **Create File** button.

EMQX Neuron currently supports two ways to upload configuration files: uploading files or providing file names and text content. If you choose **Custom File**, please provide both the file name in the file name field, such as `my.json`, and then upload or directly paste the file content in the file content field.

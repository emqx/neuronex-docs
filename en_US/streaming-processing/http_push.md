...
If you use Docker to deploy EMQX Neuron, you need to add the `-p 10081:10081` parameter to the `docker run` command to map the 10081 port in the container to the 10081 port of the host before you can use the `HTTP Push` source normally.
...
HTTP Push sources support lookup tables. Log in to EMQX Neuron and click **Data Processing** -> **Source**. On the **Scan Table** tab, click **Create Scan Table**.

...
可查看驱动节点以及规则的运行状态统计信息，深入了解 EMQX Neuron 运行情况。
...
EMQX Neuron 支持基于 Prometheus 的数据模型的指标统计功能，用于监测**驱动**、**规则**的运行状态。Prometheus 的基本原理是通过 HTTP 协议周期性抓取被监控组件的状态，任意组件只要提供对应的 HTTP 接口就可以接入监控，EMQX Neuron 提供对应的接口，可以顺利接入 Prometheus 进行监控。

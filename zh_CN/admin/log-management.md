...
NeuronEX 支持在 web 页面一键下载所有日志文件的功能，登录 NeuronEX 后，点击页面顶部功能确的**系统信息** -> **日志**，即可下载日志。
...
NeuronEX 支持设置打印某个节点的 debug 日志，在每个节点的 `更多` 操作按键中都有一个 `DEBUG 日志` 的操作按键，如下图所示，点击即可将日志级别设置为 debug 并在大致十分钟后自动切回默认的日志等级。

此时，该节点开始打印 debug 日志，用户可选择在十分钟后下载日志，查看对应节点的日志，也可以选择在 /build/logs 下实时查看节点打印的日志。

::: tip
打印节点 debug 日志的同时 NeuronEX 日志也会打印，并在十分钟后自动切回默认日志等级。
:::

...
Modbus RTU 协议采用二进制编码，可以在 RS-232、RS-485 或其他串行通信介质上传输数据，NeuronEX 的 Modbus RTU 插件增加了基于以太网 TCP 的实现，通过 DTU 设备，可实现远端设备的数据采集与控制。

## 插件配置

NeuronEX 支持基于串口或以太网建立实现远端设备的数据采集与控制。对应的参数配置如下：
...
| **连接超时时间** | 等待设备返回指令响应的时间                                   |
| **指令发送间隔** | 发送每条读写指令之间的等待时间。某些串口设备在较短时间内接收到连续指令时，可能会丢弃某些指令。 |
| **IP 地址**      | 设备的 IP 地址（NeuronEX 作为客户端）；或是 NeuronEX 的 IP 地址（NeuronEX 作为服务端），默认可填 0.0.0.0。 |
| **端口号**       | TCP 连接时，设备的端口号（NeuronEX 作为客户端）；或是 NeuronEX 本机的端口（NeuronEX 作为服务端）。 |
| **连接模式**     | 设置 NeuronEX 作为客户端（Client）或服务器端（Server）       |

...
NeuronEX 的 Modbus TCP 插件除了支持以 TCP 客户端的模式主动接入设备进行数据采集与空；还增加支持了 TCP 服务端模式，允许设备主动接入到 NeuronEX，主要用于如 4G DTU，因 4G 网络的 IP 是属于私网 IP，此时只能由 DTU 设备主动连接到 NeuronEX。
...
| **IP 地址** | TCP 连接时，设备的 IP 地址（NeuronEX 作为客户端）；或是 NeuronEX 本机的 IP 地址（NeuronEX 作为服务端），默认可填 0.0.0.0。 |
| **端口号** | TCP 连接时，设备的端口号（NeuronEX 作为客户端）；或是 NeuronEX 本机的端口（NeuronEX 作为服务端）。 |
...
> <span>SLAVE!ADDRESS\[.BIT][#ENDIAN]\[.LEN\[H]\[L]\[D]\[E]]</span>
...
| Input Register（输入寄存器） | 300001 ~ 365536 | 读          | 16Bit,2Byte         | 0x04          | BIT, INT16, UINT16, INT32<br>UINT32, INT64, UINT64,<br>FLOAT, DOUBLE, STRING |
| Hold Register（保持寄存器）  | 400001 ~ 465536 | 读/写       | 16Bit,2Byte         | 0x03,0x06,0x10 | BIT, INT16, UINT16, INT32, UINT32,<br>INT64, UINT64,<br>FLOAT, DOUBLE, STRING |

::: tip
一些设备文件会使用功能码和寄存器地址来描述指令，因为寄存器地址编号是从 0 开始的，所以每个区域的寄存器地址范围为 0 ～ 65535。NeuronEX 采用 PLC 组态地址规范，因此 NeuronEX 配置的地址从 1 开始。
组态地址规范转换规则：根据功能码确定地址的最高位数，并在寄存器地址上加 1，作为 NeuronEX 的使用地址。
:::

例如，功能码是 0x03，寄存器地址是 0，NeuronEX 使用的地址是 400001。功能码是 0x02，寄存器地址是 5，NeuronEX 使用的地址是 100006。

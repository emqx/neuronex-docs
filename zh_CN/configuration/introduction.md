...
先安装或确认插件，再添加南向驱动、配点和监控。北向上报见 [数据转发](./north-apps/north-apps.md)。各协议参数和示例见 [南向驱动协议](../introduction/plugin-list/plugin-list.md)。

建议按侧边栏顺序阅读：

1. [管理插件模块](./ecp_edge_plugin.md)：安装或替换自定义插件
...
1. [管理插件](./ecp_edge_plugin.md)：确认目标协议插件已安装。

...
EMQX Neuron 支持 MQTT 应用作为其数据汇聚上报的方式之一。 EMQX Neuron Azure IoT 应用基于 [MQTT 应用]，提供对 Azure IoT Hub 的便捷接入。
...
Azure IoT 应用通过 MQTT 主题 `devices/{device-id}/messages/events/` 接收来自 Azure IoT Hub 的写入请求，其中 `{device-id}` 是设备的**设备 ID**。

...
EMQX Neuron 支持 MQTT 应用作为其数据汇聚上报的方式之一，EMQX Neuron MQTT 应用允许用户快速构建使用 MQTT 协议的物联网应用程序，可以在设备和云之间进行通讯。开发人员还可以通过 MQTT 应用向 IoT 设备发布控制指令，触发设备操作，例如打开或关闭灯、电机或者其他设备。

此外，MQTT 应用还支持使用身份验证和加密通讯协议进行安全通讯，确保数据的安全性和隐私性。

[MQTT]: https://mqtt.org

::: tip
不建议在同一 EMQX Neuron 实例中同时启用多个 MQTT 应用节点，这可能会导致性能下降和资源竞争问题。
在同一个 MQTT 节点中，建议通过添加多个订阅来实现对不同南向设备的数据采集和转发。
:::


## 添加应用

在**数据采集 -> 北向应用**，点击 **添加应用** 添加 MQTT 客户端节点。

## 应用配置

以下是使用 MQTT 应用配置节点时可用的参数：
...
离线数据缓存是 MQTT 应用特有的功能，当网络中断发生时，MQTT 应用首先将数据存储在内存缓存中，并且仅当内存缓存已满时才将数据刷新到磁盘缓存中。当网络恢复连接时，MQTT 应用将以 FIFO（先进先出）的顺序将缓存的数据发布到服务器上。这项功能在网络连接不稳定的场景中非常有用，可以帮助防止网络离线或暂时不可用时数据丢失，增强构建在 EMQX Neuron 上的应用程序的鲁棒性和可靠性。
...
4. 通过**缓存消息重传间隔**参数指定缓存消息重传间隔（单位为毫秒），最大允许缓存消息重传间隔为 120 秒 。

下表给出了（使用[ Modbus TCP 驱动](../../south-devices/modbus-tcp/modbus-tcp.md)时）离线缓存磁盘空间使用情况的一些统计数据。
...
::: tip
此应用会自动订阅多个 Topic， 建议在 MQTT Broker 上配置 ACL 时，为每个 Topic 配置相应的权限，以确保应用能够正常工作。
:::
...
EMQX Neuron 的 MQTT 应用支持 MQTT over SSL。
...
本节将通过[公共的 EMQX Broker（broker.emqx.io）](https://www.emqx.com/zh/mqtt/public-mqtt5-broker)为例演示如何通过 MQTT 应用实现 Mobbus TCP 数据的转发。

### 创建南向驱动

南向驱动 Modbus TCP 已安装且完成组和点位的配置，和 EMQX Neuron 之间通信正常。有关 Modbus TCP 驱动的安装及配置，见 [Modbus TCP](../../south-devices/modbus-tcp/modbus-tcp.md)。

### 创建北向应用

在**数据采集 -> 北向应用**，点击 **添加应用** 添加 MQTT 客户端节点。点击应用卡片上的**应用配置**按键进入应用配置界面设置 MQTT 连接，并进行如下设置：
...
此时，我们将能在 MQTTX 中看到由 EMQX Neuron 转发的数据，如下图所示。

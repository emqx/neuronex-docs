...
下表给出了（使用[ Modbus TCP 驱动](../../south-devices/modbus-tcp/modbus-tcp)时）离线缓存磁盘空间使用情况的一些统计数据。
...
::: tip
此应用配置会，会自动订阅多个 Topic， 建议在 MQTT Broker 上配置 ACL 时，为每个 Topic 配置相应的权限，以确保应用能够正常工作。
:::

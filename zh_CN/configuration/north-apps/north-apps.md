...
南向点位采到之后，用北向应用把数据送到 MQTT、云或处理引擎。

阅读顺序：

1. 本页：创建北向节点
2. [订阅南向数据](../subscription.md)：把南向组挂到该应用
3. [北向应用协议](./catalog.md)：MQTT、Sparkplug B、Kafka 等参数和示例
...
- [DataStorage 数据存储插件](./DataStorage/DataStorage.md)：DataStorage 插件用于将采集的数据存储到外部 Datalayers 时序数据库中。

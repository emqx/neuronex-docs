...
EMQX Neuron Mitsubishi 3E 插件完全兼容 Mitsubishi SLMP 协议，主要用于通过以太网访问三菱的 QnA 兼容 PLC，包括 Q 系列（MC）、iQ-F 系列（SLMP）和 iQ-L 系列：
...
在 **数据采集 -> 南向设备**，点击**添加设备**来创建设备节点，输入插件名称，插件类型选择 **Mitsubishi 3E** 启用插件。

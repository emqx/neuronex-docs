...
| **服务器地址** | DTU 作为客户端时，填写 EMQX Neuron Modbus 绑定的地址。|
| **本地/远程端口** | DTU 作为客户端时，填写 EMQX Neuron Modbus 绑定的端口，DTU 作为服务端时，为 DTU 的端口。 |

:::tip
DTU 一般都支持串口心跳包，或者是使能网络心跳包，以及注册包，这些特性在标准 Modbus 协议中都无法使用，EMQX Neuron 目前还无法兼容处理这些特性，使用 EMQX Neuron 连接 DTU 时，注意关闭相关选项。
:::
...
## 连接作为 Client 的 EMQX Neuron

本节主要介绍 EMQX Neuron 作为 Client，DTU 作为 Server 时，EMQX Neuron 与 DTU 的相关配置。

EMQX Neuron 作为 Client，主动向 DTU 发起连接请求，用户需要保证 EMQX Neuron -> DTU 的网络连通性。

### 配置 DTU Server

首先，需要配置 DTU 与串口连接的参数，其次，需要配置 DTU 与 EMQX Neuron 建立连接的 Socket 参数，如下图所示。
...
在配置 EMQX Neuron 南向驱动时需要作为 Server 端的 DTU 的 IP，如下图所示。
...
## 连接作为 Server 的 EMQX Neuron

本节主要介绍 EMQX Neuron 作为 Server，DTU 作为 Client 时， EMQX Neuron 与 DTU 的相关配置。DTU 作为 Client，主动向 EMQX Neuron 发起连接请求，用户需要保证 DTU -> EMQX Neuron 的网络连通性。

**应用场景**

这种连接方式通常可用于以下场景中，在某些 DTU 使用 4G 上网时，因为 EMQX Neuron 无法主动连接到 DTU，所以，EMQX Neuron 只能选择 Server 模式，由 DTU 主动连接到 EMQX Neuron。

此外，当 EMQX Neuron 与 DTU 不在同一局域网内时，也可以将 EMQX Neuron 运行环境的局域网 IP 及端口映射到公网，并将 EMQX Neuron 作为 Server 端，DTU 作为 Client 端。
...
* 远程服务器地址，填写作为 Server 端运行 EMQX Neuron 的 IP 地址；
* 本地端口，默认不填写；

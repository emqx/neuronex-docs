...
OPCUA Part9 条件和报警功能，必须在 Subscribe 模式下使用。
...
**发布间隔**定义了当**更新模式**设置为 Subscribe 或 Read&Subscribe 模式时， OPCUA 服务器向 EMQX Neuron 循环发送点位更新值的最快速率。如果该参数设置为0，使用 OPCUA 服务器支持的最快发布间隔。该间隔以毫秒为单位表示。默值值为 500ms，表示 OPC UA 服务器发送给 EMQX Neuron 最快的点位更新速率为 500ms。

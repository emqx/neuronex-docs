...
该驱动的点位规模与采集周期实测数据，见[OPC UA 驱动性能测试](../../../performance/opcua.md)。
...
OPC UA Part9 条件和报警功能，必须在 Subscribe 模式下使用。

:::

## 添加驱动

在 **数据采集 → 南向设备** 页点击 **添加设备**，驱动类型选择 **OPC UA**。
...
**更新模式**在南向节点的全局参数中设置，表示该南向节点以 OPC UA 的订阅方式获得数据。而 EMQX Neuron 点位设置中的 Subscribe 属性表示这个点位的数值发生变化时立即触发北向应用的上送报告。同时设置二者便可以获得实时的数据变化报告。

当**更新模式**配置为 Subscribe 或者 Read&Subscribe 时，点位的属性配置为 Subscribe 时，则采集组的 Interval 对该点位不生效，即点位不会周期上报数据。

当**更新模式**配置为 Subscribe 或者 Read&Subscribe 时，点位的属性配置为 Read 加 Subscribe 时，则采集组的 Interval 对该点位生效，即点位也会周期上报数据。
...
- EMQX Neuron 设置的数据类型必须与 OPC UA 数据类型相匹配。
...
OPC UA Extension Object 支持数组和嵌套。

条件，报警和 Method 的类型都为 JSON。
...
| Float                    | FLOAT                             |
| Double                   | DOUBLE                            |
| Boolean                  | BOOL （值为 true 或 false） |
| Boolean                  | BIT （值为 0 或 1）         |
| String                   | STRING                            |
| Datetime                 | UINT32                            |
| LocalizedText（只读） | STRING                            |
| SByte Array              | ARRAY_INT8                        |
...
ARRAY_CHAR 以字符串形式显示和写入。
JSON 以 JSON 字符串形式显示和写入。
OPC UA Extension Object 支持数组与嵌套。

### 地址格式

> (cond:|alarm:|method:)NS[x,y,z]!NODEID(Method-NS!Method-NODEID)(?para1=type1&para2=type2 ...)

**cond:|alarm:|method:** 条件，报警和 Method 特殊节点标志
...
**Method-NS!Method-NODEID** Method 节点的名字空间索引和节点 ID。

**para1=type1&para2=type2** Method 调用需要的参数名和参数类型，目前支持 Boolean，Sbyte， Byte， Int16， UInt16， Int32， UInt32， Int64， UInt64， Float， Double， String， ByteString， Guid， LocalizedText 这些类型。
...
| method:1!TemperatureAlarm(0!9027)?                                         | JSON     | Method 节点                                                                                                                |
| method:1!TemperatureAlarm(0!9111)?EventId=ByteString&Comment=LocalizedText | JSON     | Method 节点                                                                                                                |

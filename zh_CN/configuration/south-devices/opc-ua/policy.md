...
* 匿名方式
    ...
    EMQX Neuron OPC UA 模块无需设置用户名/密码和证书/密钥。

* 用户名/密码方式 

    OPC UA 服务端已经创建好具备访问权限的用户名/密码。

    EMQX Neuron OPC UA 模块填写对应的用户名/密码，无需添加证书/密钥。

* 证书/密钥 + 匿名方式

    OPC UA 服务端开启合适的安全设置，添加客户端证书到信任列表，并且开启匿名登录。

    EMQX Neuron OPC UA 模块添加对应的客户端证书/密钥，无需填写用户名/密码。

* 证书/密钥 + 用户名/密码方式

    OPC UA 服务端已经创建好具备访问权限的用户名/密码，开启合适的安全设置，并添加客户端证书到信任列表。

    EMQX Neuron OPC UA 模块添加对应的用户名/密码，添加对应的客户端证书/密钥。

## 安全模式与安全策略

当使用证书认证方式连接 OPC UA 服务器时，需要在 EMQX Neuron 中配置**安全模式（Security Mode）**，并确保服务器端启用了匹配的**安全策略（Security Policy）**。
...
安全策略定义了签名和加密所使用的具体密码学算法。当 EMQX Neuron 以 **Sign** 或 **Sign&Encrypt** 模式连接服务器时，客户端和服务器会自动协商一个双方都支持的安全策略。以下是常见的安全策略：
...
在 EMQX Neuron 中，用户仅选择**安全模式**（`None`/`Sign`/`Sign&Encrypt`），无需也不能选择具体的安全策略。策略选择由OPC UA 驱动自动完成。

**客户端初始化：** EMQX Neuron 启动 OPC UA 连接时，会注册驱动支持的所有内置安全策略：
- None
- Basic128Rsa15
- Basic256
- Basic256Sha256
...
:::tip
如果安全模式选择了 `Sign` 或 `Sign&Encrypt`，但没有上传证书和密钥，EMQX Neuron 会使用内置的默认证书和密钥。
:::

**连接协商（首次匹配即选中）：** 连接服务器时，流程如下：

1. EMQX Neuron 会获取服务器支持的所有端点列表，包括每个端点的 Security Mode 和 Security Policy。
2. 客户端按服务器返回的顺序逐一遍历端点。
3. 对每个端点检查两个条件：
   - 端点的 `securityMode` 必须与 EMQX Neuron 中配置的安全模式一致。
...
- 如果找不到匹配的端点，连接将失败。请确保服务器至少有一个端点的安全模式与 EMQX Neuron 配置一致，且其安全策略在 驱动支持列表中。
:::
...
:::tip
证书文件可以提前导入到目标服务器中并设置为信任，也可以由 EMQX Neuron 设置后自动提交再由服务端设置为信任。
:::
...
当 EMQX Neuron 使用 **Sign** 或 **Sign&Encrypt** 模式时，OPC UA 服务器端必须进行以下配置：
1. 启用所需的安全策略。
2. 信任上传至 EMQX Neuron 的客户端证书。

以下以常用的工业 OPC UA 服务器为例，介绍具体配置步骤。

### 通用配置原则

1. 使用 OpenSSL **生成客户端证书并上传至 EMQX Neuron**（参考[客户端证书/密钥生成](#客户端证书密钥生成)）。
...
4. 点击**确定**保存端点配置。
5. 客户端证书上传至 EMQX Neuron 后，右键点击 KEPServerEX 图标，选择 **OPC UA 配置 -> 受信任的客户端**。
6. 点击**导入**，浏览选择 EMQX Neuron 客户端证书文件（DER 格式），将其添加到信任列表。

...
> 由于本协议的特殊性，新建点位时需要保证点位配对，但主动上报的事件可以单独新建点位，如 S6F3。

## 特殊类型处理

SECS-II 消息定义了 LIST 类型，驱动也通过 string 支持该类型，但是有一个反序列化的过程，具体的规则如下所示。
...
LIST 类型支持使用下标访问具体的元素，如 S6F3[1][1][1]，目前支持嵌套十层 LIST。
...
本驱动 LIST 中 ASCII 序列化为 `<A[n] xxx>`，内容不带引号。例如 "ABC" 序列化为 `<A[3] ABC>`，而不是 `<A[3] "ABC">`。

* LIST 中 Binary 类型如何序列化？
本驱动 LIST 中 Binary 序列化为 `<B[n] xx>`，内容不带引号。例如 0xFF 序列化为 `<B[1] FF>`，而不是 `<B[2] "FF">`。
...
例如 `S1F1`，其不需要传递参数，但仍需要新建配对的点位。比如 `S1F1` 发送 Hello 消息后，设备会通过 `S1F2` 返回数据，因此需要同时创建 `S1F1` 和 `S1F2` 两个点位。

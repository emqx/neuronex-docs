...
EMQX Neuron 的数据探索模块是您洞察工业历史数据、发现价值的核心工具。它集成了强大的数据查询、交互式分析以及可定制的可视化仪表盘功能，帮助您轻松理解设备行为、优化生产流程、并基于数据做出更明智的决策。
...
EMQX Neuron v3.6.0 版本开始，**打包集成了 Datalayers 时序数据库**。这意味着 **多种安装包形式:** `deb`, `rpm`, `zip`, `docker` 等，均已内置 Datalayers 时序数据库。

要开始使用数据探索模块：

1. 请确保您已在 EMQX Neuron 中开启 [时序数据存储](../admin/sys-configuration.md#数据存储配置)。Datalayers 默认不随 EMQX Neuron 启动。您需要在 EMQX Neuron 的系统配置中按需开启此服务。
...
通过 EMQX Neuron 的数据探索模块，释放您工业数据的真正潜力！
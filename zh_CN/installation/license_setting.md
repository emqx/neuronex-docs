...
NeuronEX 的许可证提供硬件绑定与硬件不绑定两种方式。 在许可证页面可看到当前运行 NeuronEX 的硬件标识。当 NeuronEX 的许可证为硬件绑定类型时，该许可证仅支持在硬件标识匹配的 NeuronEX 上生效。

:::tip
在 Docker 等容器方式安装的 NeuronEX 中，硬件标识为空，无法进行硬件绑定。
:::

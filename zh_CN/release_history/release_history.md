...
- 新增 Mazak CNC 驱动，该驱动通过被动 UDP 监听方式采集 Mazak CNC 的实时运行数据。

- 新增 Brother CNC 驱动，该驱动用于通过 Brother NC 协议访问 Brother（兄弟）数控加工中心，采集机床的坐标、主轴、刀具、报警、计件、程序号、PLC 软元件以及刀具补偿等运行数据。

- 北向驱动 OPCUA Server 完成安全能力优化，移除内置固定证书，支持证书动态生成，提升服务安全性与灵活性

- OPCUA 驱动拓展数据类型适配能力，新增支持 DateTime、Localized Text 数组类型数据解析与接入

- Allen-Bradley 5000 EtherNet/IP 驱动功能优化

  - 新增批量点位写入能力，支持通过API或neuron Sink进行多点位同时写入。

  - 同时优化批量读点位逻辑，单个点位读取报错尽量不影响其他正常点位的采集结果。

- 优化北向 MQTT 驱动静态点位数据适配能力，原有仅支持 String 类型数据，现已扩展支持 Float、Bool、Int 多种常用数据类型。
...
- **Modbus TCP/RTU 驱动**：支持使用 **0x16** 功能码写入保持寄存器 bit 位。

### **修复**

- 修复 **OPC UA Server** 在 Neuron 配置大量节点时崩溃的问题。

- **MQTT/SparkplugB** 开启 TLS 时强制配置 CA，避免开启 TLS 后实际未使用 TLS 连接。

- 修复 **ADS 驱动**读取设备数据失败时可能导致 CPU 使用率达到 **100%** 的问题。

- 修复多个模块的内存异常问题，包括堆栈越界、数值越界、内存信息泄漏。

- 修复 **RBAC** 功能中存在的问题。

- 修复日志中的敏感信息泄露问题。

- 修复 **Node-RED** 页面无法打开的问题。

- 规范 **Okta SSO** 前端参数，后端延长超时时间。
...
- **OPC UA 驱动文档**：新增并完善安全策略 **Sign** / **Sign&Encrypt** 相关说明。

- **Modbus TCP/RTU 驱动**：新增标准 **0x16** 功能码写入支持。

- **南向驱动采集**：采集通道最大采集周期由 **100ms** 调整至 **30ms**。若业务存在高频采集需求，可手动修改该配置项；需注意，过高的采集频率会受设备协议、采集点位数量制约，易引发采集异常。高频采集场景的调优方案，可联系 EMQ 技术团队获取专业支持。

- **MQTT 插件**：统一 Topic 前缀规则，删除 Topic 首字符斜杠 `/`。

- **点位配置**：偏移量属性移除 **1000** 的最大值限制。

- **点位测试**：优化提示文案展示。

- **添加点位页面优化**：添加点位时若点位配置错误，高亮该点位并提示错误原因；点位描述信息去除弹窗展示；点位描述列支持拖动调整列宽并加深分割线颜色。

### **修复**

- 修复北向 MQTT 插件连接异常问题，完善异常日志输出。

- 修复 **Allen-Bradley 5000 EtherNet/IP** 驱动 String 类型奇数长度缺少填充 0 的问题。

- 修复 **Fanuc Focas Ethernet** 驱动报警信息显示失败问题。

- 修复北向 **Kafka** 插件添加订阅时 UI 展示异常问题。

- 修复点位重命名后编辑该点位提交报错的问题。

- 修复添加点位时前端错误提示未携带接口返回索引信息的问题。
...
- 修复了使用 MQTT 协议版本 5 时 SSL 连接失败的问题。

- 修复了共享 Source 通配符规则分离时 Schema 丢失的问题。当两个或多个规则共享同一个 SHARED=true 的数据源（如 neuronStream），且均使用通配符查询（SELECT *）时，停止其中一个规则会导致剩余规则开始输出空元组 {}。
...
- **可观测性**: 为数据处理模块的数据源共享连接添加了可观测性能力。
- **日志**: 增加了连接池日志，以便更好地进行故障排查。

### **修复**

- 修复了一个难以复现的竞争条件问题。该问题曾可能导致在使用共享连接时，由于特定错误引发子拓扑（或规则）假死或挂起。
- 修复 Kafka Sink 传输资源泄漏：当规则停止时，Kafka sink 底层的传输协程（特别是 discover 协程）无法终止。这是由于 kafka-go writer 不会自动关闭外部传输连接导致的。现在会显式调用 CloseIdleConnections() 以确保所有资源都能被正确释放。
...
- **添加点位交互优化**：添加点位时，点位地址列支持拖动调整列宽；添加点位时，未点击保存即离开页面，新增弹窗提示。
...
- **点位写入增加日志**： 通过 MQTT 或 HTTP API 执行点位写入操作时，在日志中新增接口执行状态（成功 / 失败）及报错详情（若有）。执行如果成功，则为 INFO 日志，执行如果失败，则为 ERROR 日志。

- **MQTT 驱动增强**：当北向 MQTT 驱动节点删除，或者离线缓存功能关闭时，离线缓存数据将被清除。

- **Allen-Bradley 5000 EtherNet/IP 驱动**：支持CompactLogix 53xx系列的 Struct String 类型写入。

- **IEC61850-5-101 驱动**：支持携带 cp24 的类型，支持配置参数长度。

- **SECS GEM 驱动**：List 结构支持嵌套层数从4层增加到10层。

- **优化新增点位交互**：点位名称列支持拖动调整列宽，适配超长点位名称展示。

- **REST Sink**：新增对 REST Sink 输出时 Access Header（授权头）的支持。

- **备份与恢复功能**：3.7.2及后续版本支持导出的备份文件支持重命名后导入。（3.7.1及之前版本，不支持备份文件重命名后导入）

### **修复**

- 修复 OPC UA 更新模式配置为 Read 或者 Subscribe 时，点位读取失效的检测逻辑，并自动重连 OPC UA Server 和读取点位。

- 修复 OPC UA 点位浏览功能 Node 重复的问题。

- 修复 OPC UA 点位浏览功能，只过滤 Method 节点。

- 集成最新版 MQTT SDK，优化 QoS 2 消息机制阻塞问题；修复大报文传输（单采机组点位过多）触发 MQTT Broker 连接断开的问题。

- 修复 Sparkplug B 驱动，触发rebirth导致的崩溃。

- 修复 Viewer 角色用户无法访问数据监控页面的问题。

- 修复 Viewer 角色用户在数据分析、仪表盘等页面权限不对的问题。

- 导出的备份文件不能重命名，重命名后导入会报错
...
- 修复 OPC UA 订阅任务时间戳溢出，导致任务更新失败的问题。

...
EMQX Neuron 数据处理模块通过 `CAN` 类型的数据源，可以接收来自 CAN 协议总线的数据，将原始数据解析为结构化数据。CAN 类型可以作为流、扫描表的数据源。
...
以下图为例，EMQX Neuron 通过网卡 can0 接收数据，并根据`/home/dbc`目录下的.dbc文件，将 can 数据解析为结构化 json 数据。

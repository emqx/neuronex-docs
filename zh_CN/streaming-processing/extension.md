...
EMQX Neuron 允许用户自定义算法集成，以支持更多功能。 用户可以通过便携插件系统编写扩展插件，也可以调用外部已有的 REST 服务。


## 便携插件

您可通过安装便携插件进一步拓展 EMQX Neuron 的功能。通过便捷插件可以实现自定义的**数据源(Source)**、规则中的**自定义函数**以及**动作(Sink)**。
...
:::tip 提示
Python 便携插件依赖 Python 3 与 eKuiper Python SDK。**请使用标准 Docker 镜像 `emqx/neuronex:x.y.z`。** `*-slim` 镜像以及未自行安装 Python 的二进制包无法完成插件进程握手。详见 [部署要求](portable_python.md#部署要求) 和 [Docker 容器 Python 运行环境](../installation/docker.md#选择镜像)。
:::
...
外部算法函数，即外部服务，是指将外部已有的一个 HTTP 服务，映射为 EMQX Neuron 的一个 SQL 函数。在规则中使用外部函数时，会将数据源中的数据传递给外部服务，并将外部服务的计算结果返回到 EMQX Neuron 并做输出。
...
外部函数扩展，即外部服务，是指通过提供一种配置的方式，使得 EMQX Neuron 可以使用 SQL 以函数的方式直接调用外部服务，包括各种 rpc 服务、 http 服务等。该方式将可大大提高 EMQX Neuron 扩展的易用性。外部函数将作为插件系统的补充，仅在性能要求较高的情况下才建议使用插件。

以 getFeature 函数为例，假设有 AI 服务基于 grpc 提供 getFeature 服务。则可在 EMQX Neuron 配置之后，使用 `SELECT getFeature(self) from demo` 的方式，无需定制插件，即可调用该 AI 服务。
...
外部函数通过注册外部服务配置的方式，将已有的服务映射成 规则引擎应用 SQL 函数。运行使用外部函数的规则时，规则引擎应用 将根据配置，对数据输入输出进行转换，并调用对应的服务。用户通过**外部服务**页签注册外部服务后，函数将出现在**外部函数列表**页。
...
完成上述设置后，点击**提交**完成插件的创建。新建插件将出现在该页面的插件列表中，您可以在此查看或者删除插件。关于 EMQX Neuron 目前支持以下类型的扩展：

- [数据源](./source.md)：为 EMQX Neuron 添加新的源类型，以便从中获取数据。新的扩展源可以在流/表定义中使用。
- [数据汇](./sink/sink.md)：为  EMQX Neuron 增加新的 sink 类型来发送数据。新的扩展 sink 可以在规则动作定义中使用。
- 函数：为 EMQX Neuron 添加新的函数类型来处理数据。新的扩展函数可以在规则 SQL 中使用。 -->

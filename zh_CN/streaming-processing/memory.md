...
内存源获取 EMQX Neuron 流处理模块内存中保存的数据流事件。内存动作的典型用途是形成 [**规则流水线**](./rule_pipeline.md)，将上一个规则处理的结果放在 [**内存 Sink**](./sink/memory.md) 中，然后再通过内存源获取数据，形成规则流水线。
...
内存源支持查询表。登录 EMQX Neuron，点击**数据处理** -> **源管理**。在**扫描表**页签，点击**创建扫描表**。
...
登录 EMQX Neuron，点击**数据处理** -> **源管理**。在**查询表**页签，点击**创建查询表**。

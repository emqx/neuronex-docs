...
NeuronEX 为 Neuron 源流提供了内置支持，流可以订阅来自本地 Neuron 的消息并输入 NeuronEX  处理。

:::tip

该源仅可用于本地的 Neuron，因为与 Neuron 的通信基于 nanomsg ipc 协议，无法通过网络进行。在 NeuronEX 端，所有 Neuron 源和动作共享同一个 Neuron 连接。

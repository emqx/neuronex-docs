...
EMQX Neuron 3.8.0 将 AI 能力深度集成到数据处理流程中，显著降低了工业数据分析的技术门槛。

创建规则页面集成 AI 问答
在创建数据处理规则时，AI 助手现已深度集成 eKuiper 相关知识库，支持通过自然语言咨询 SQL 规则编写及数据流处理问题。用户可以直接询问各类问题，AI 会生成对应的 SQL 代码。例如：

- 我想检测设备“连续 3 次温度（temp）超过 100 度”的异常情况，应该如何结合窗口和过滤规则来实现？

...
对窗口内的数据做聚合计算时，默认做法是把流数据按窗口定义切分、在内存中缓存整个窗口的数据，等窗口结束后一次性计算。窗口较大或数据速率较高时，这批尚未参与计算的数据会撑大内存，严重时引起 OOM。

部分聚合函数并不需要保留原始数据。以 `avg` 为例，每读到一条数据只需更新 sum 和 count 两个中间状态，窗口结束时相除即可。增量计算就是用这种方式替换默认实现：数据来一条算一条，窗口内只保留中间状态。

SQL 写法不变，仍然写 `count()`、`sum()`、`avg()`，是否走增量路径由执行计划决定。

在规则选项中打开**启用增量计算**：

![规则选项中的启用增量计算开关](_assets/incremental_calc.png)

::: tip
增量计算要求规则**同时**包含时间窗口和支持增量的聚合函数。若 SQL 中出现不支持增量计算的聚合函数（例如 `stddev`），整条规则会退回默认的窗口实现，该开关不起作用。
:::

...
在 NeuronEX 端，所有的 Neuron 源和动作共享同一个全局 Neuron 连接。注意：拨号到 Neuron 是异步的，它将在后台运行，不断重拨直到连接成功，因此用户无法在 Neuron 停机时及时看到错误提示。您可通过查看消息流入数量判断规则的运行状况。

此外，在 nanomsg 消息客户端中有一个 128 条消息的发送队列，因此即使 Neuron 停机，Neuron sink 也能有 128 条消息发送出去。规则只有在 128 条消息队列满之后才会开始出现超时错误。一旦连接恢复，队列中的消息就会自动发送出去。<!--【attention：这块需要解释下】-->

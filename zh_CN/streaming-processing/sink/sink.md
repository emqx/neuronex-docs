...
在 NeuronEX，Sink 用来向外部系统写入数据，分为内置动作和扩展动作两类。
...
离线缓存的保存位置根据 `etc/kuiper.yaml` 里的 store 配置决定，默认为 sqlite，此时所有的缓存将被保存到 `data/cache.db` 文件。每个 sink 将有一个唯一的 sqlite 表来保存缓存。缓存的计数添加到 sink 指标中的 buffer length 部分。

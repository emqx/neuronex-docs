...
有五种窗口可供使用： [滚动窗口](#滚动窗口)， [跳跃窗口](#跳跃窗口)，[滑动窗口](#滑动窗口)，[会话窗口](#会话窗口)和[计数窗口](#计数窗口)。 您可以在查询语法的 GROUP BY 子句中使用窗口函数。
...
跳跃窗口每次在时间上向前跳一段固定的距离。可以把它理解为允许重叠的滚动窗口，因此同一个事件可能落入多个跳跃窗口的结果集。将跳跃大小设为与窗口大小相同时，跳跃窗口等同于滚动窗口。
...
filter 子句必须跟在 window 函数后面。filter 子句必须类似于 `FILTER(WHERE expr)`。例如：
...
每个事件都有一个与之关联的时间戳。 时间戳将用于计算窗口。 默认情况下，当事件输入到源时，将添加时间戳，称为`处理时间`。 也支持将某个字段指定为时间戳，称为`事件时间`。 时间戳字段在流定义中指定。 在下面的定义中，字段 `ts` 被指定为时间戳字段。
...
支持事件时间的流处理器需要一种衡量事件时间进展的方式。例如创建一个一小时的时间窗口时，内部算子需要在事件时间越过一小时后得到通知，才能发出正在进行的窗口。

EMQX Neuron 衡量事件时间进展的机制是**水印（Watermark）**。水印作为数据流的一部分传递，带有一个时间戳 `t`。水印 `t` 声明事件时间在该数据流中已经到达 `t`，意味着此后不应再出现时间戳 `t' <= t` 的元素。

水印在规则层面维护，因此一条规则从多个数据流读取数据时，水印会在其全部输入流中流动。
...
对于滑动窗口，每一条数据都可以触发一个窗口，可通过 `over` 子句将触发窗口的数据进行过滤，只会将满足过滤条件的数据去触发窗口。`over` 子句可以单独用在滑动窗口后面，也可以用在 `filter` 子句后，`over` 子句必须类似于 `Over(When expr)`，例如:
